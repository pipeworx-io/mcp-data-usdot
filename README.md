...
`tools/list` at `https://gateway.pipeworx.io/data-usdot/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/data_usdot_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"traffic accidents"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/data_usdot_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-usdot": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-usdot"]
    }
  }
}
```
